...
export class CreateResourcePlannerComponent implements OnDestroy {
  protected loadingState: LoadingState = { isLoading: false };

  protected organization?: OrganizationDto;
  protected resourcePlannerName = new FormControl('', { nonNullable: true });
      ...
      this.turnierplanApi
        .invoke(createResourcePlanner, {
          body: {
            ...
            name: this.resourcePlannerName.value
          }
        })
        .pipe(
          switchMap((resourcePlanner) =>
            from(this.router.navigate(['../../../../resource-planner/', resourcePlanner.id], { relativeTo: this.route }))
          )
        )
        .subscribe({
          next: () => {
