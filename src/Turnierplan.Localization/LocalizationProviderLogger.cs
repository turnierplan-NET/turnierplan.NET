...
internal sealed partial class LocalizationProviderLogger
{
    ...
    [LoggerMessage(LogLevel.Error, "Stream for translations file '{LocalizationResourceName}' is null.", EventId = 100)]
    public partial void LocalizationResourceStreamIsNull(string localizationResourceName);

    [LoggerMessage(LogLevel.Warning, "Could not find CultureInfo for language code '{LanguageCode}'. Falling back to 'InvariantCulture'.", EventId = 101)]
    public partial void CouldNotFindCultureInfoForLanguageCode(string languageCode);

    [LoggerMessage(LogLevel.Error, "Failed to load translations for language with code '{LanguageCode}'.", EventId = 102)]
