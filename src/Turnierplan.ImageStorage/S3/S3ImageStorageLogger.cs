...
internal sealed partial class S3ImageStorageLogger
{
    ...
    [LoggerMessage(LogLevel.Error, "Failed to upload image '{ObjectKey}' to S3. Result status code: {StatusCode}", EventId = 101)]
    public partial void FailedToUploadImage(string objectKey, int statusCode);

    [LoggerMessage(LogLevel.Error, "Failed to upload image '{ObjectKey}' to S3 because of an exception.", EventId = 102)]
    public partial void FailedToUploadImage(Exception exception, string objectKey);

    [LoggerMessage(LogLevel.Error, "Failed to read image '{ObjectKey}' from S3. Result status code: {StatusCode}", EventId = 103)]
    public partial void FailedToReadImage(string objectKey, int statusCode);

    [LoggerMessage(LogLevel.Error, "Failed to delete image '{ObjectKey}' from S3 because of an exception.", EventId = 104)]
