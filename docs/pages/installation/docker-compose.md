...
Die einfachste Möglichkeit, turnierplan.NET auf einem lokalen Rechner, einer VM oder einem dedizierten Server zu installieren, ist mithilfe von Docker Compose. Die nachfolgende YAML-Definition beinhaltet die notwendigen Container für Datenbank sowie den Anwendungsserver:
...
Die Anmeldung als Administrator erfolgt mit den Zugangsdaten, welche beim ersten Programmstart generiert und in den Container-Logs ausgegeben werden. Weitere Schritte sind auf der entsprechenden Seite [Erste Schritte](../getting-started/index.md) der Dokumentation beschrieben.

!!! tip
    Zum Abrufen der Container-Logs kann der Befehl `docker compose logs turnierplan.application` verwendet werden.
